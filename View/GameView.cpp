...
GameView::GameView() : window(sf::VideoMode::getDesktopMode(), "C++ Project", sf::Style::Default) {
    ...
    enemySprite.setScale(1.0f, 1.0f);  // Scale as needed.
...
void GameView::renderCards(const std::vector<Card>& cards, int selectedCardIndex) {
    ...
    for (size_t i = 0; i < numCardsToDisplay; ++i) {
        ...
        sf::Color cardColor;
        if (card.getType() == "Att") {
            cardColor = sf::Color(255, 0, 0);  // Red for attack cards.
        } else if (card.getType() == "Heal") {
            cardColor = sf::Color(255, 182, 193);  // Pink for healing cards.
        } else if (card.getType() == "Def") {
            cardColor = sf::Color(0, 255, 0);  // Green for defense cards.
        } else if (card.getType() == "Bonus") {
            cardColor = sf::Color(0, 0, 255);  // Blue for bonus cards.
        } else {
